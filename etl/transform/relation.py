  ...
  def insert_config_bulk_no_extra_props(self, docs, attrs, include_extra_props=True, unset=[]):
    ...
    for doc in docs:
      ...
      if len(docs) > 1:
        ...
        attrs_pg = [v["name_conf"] for k, v in attrs.items() if (
            k in doc.keys() or k in attrs.keys())]
        values = [v["value"] for k, v in attrs.items() if (
            k in doc.keys() or k in attrs.keys())]
      result.append(tuple(values))
  ...
  def get_attrs_and_vals(self, attributes, doc):
          ...
          if type_orig != type_new:
            ...
            if attr_new is not None:
              if attr_new not in self.column_names:
                table.add_column(self.db, self.schema,
                                 self.relation_name, attr_new, type_new)
    ...
    if len(self.column_names) == 0:
      ...
      table.add_multiple_columns(
          self.db, self.schema, self.relation_name, reduced_attributes, types)
  ...
  def udpate_types(self, attrs_types_conf):
      ...
      if self.exists() is True and column_info is not None:
        ...
        if set(attrs_conf).issubset(set(attrs_pg)):
              ...
              if type_old == 'timestamp without time zone' and type_new == 'timestamp':
                ...
                if self.is_convertable(type_old, type_new):
                  table.column_change_type(
                      self.db, self.schema, self.relation_name, attrs_pg[i], type_new)
                else:
          ...
          table.add_multiple_columns(
              self.db, self.schema, self.relation_name, attrs_to_add, types_to_add)
      else:
