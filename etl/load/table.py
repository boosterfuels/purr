...
def exists(db, schema, table):
    ...
    False: otherwise
    """
    cmd = "SELECT table_name FROM information_schema.tables WHERE table_schema='%s' AND table_name='%s';" % (
        schema, table.lower())
...
def add_column(db, schema, table, column_name, column_type):
    ...
    logger.warn("[TABLE] Adding new column to table: %s, column: %s, type: %s" % (
...
def column_change_type(db, schema, table, column_name, column_type):
    ...
    logger.warn("[TABLE] ALTER TABLE %s, changing type of column '%s' to '%s'" % (
        table.lower(), column_name, column_type))
    ...
    try:
        ...
        logger.error(
            '[TABLE] ProgrammingError: %s when executing command %s.' % (ex, cmd))
    except Exception as ex:
        logger.error('[TABLE] %s when executing command %s.' % (ex, cmd))
