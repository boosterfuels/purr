...
class Extractor():
    ...
    def prepare_transfer_conf(self, coll_names_in_config):
        """
        Transfers documents or whole collections if the number of fields is less than 30 000 (batch_size).
            ...
            try:
                if (i+1) % nr_of_transferred == 0 and i+1 >= nr_of_transferred:
                    if self.include_extra_props is True:
                        r.insert_config_bulk(
                            transferring, attrs_details, self.include_extra_props)
                    else:
                ...
                if i + 1 == nr_of_docs and (i + 1) % nr_of_transferred != 0:
                    if table.exists(self.pg, self.schema_name, r.relation_name):
                        ...
                        logger.error(
                            '[EXTRACTOR] Table %s might be deleted.' % r.relation_name)
                        return
    ...
    def transfer_doc(self, doc, r, coll):
        '''
        Transfers single document when tailing.
        Parameters
        ----------
        doc : dict
            : document 
        ...
        attrs_details = self.prepare_attr_details(
        ...
        try:
            ...
            if self.include_extra_props is True:
                ...
                r.insert_config_bulk_no_extra_props(
                    [doc], attrs_details, self.include_extra_props)
        except Exception as ex:
    ...
    def prepare_attr_details(self, attrs_conf, attrs_mdb, types_conf, type_extra_props_pg=None):
        ...
        '''
        print("BXX", self.include_extra_props)
        if self.include_extra_props is True:
    ...
    def adjust_columns(self, coll):
        ...
        if self.include_extra_props is True:
            ...
            if r.exists() is True:
                table.add_column(
                    self.pg, self.schema_name, r.relation_name, name_extra_props_pg, type_extra_props)
        else:
            if r.exists():
                # Remove column extra_props from table if it exists
                table.remove_column(
                    self.pg, r.relation_name, name_extra_props_pg)
        ...
        if type_update_failed is not None:
            for tuf in type_update_failed:
                name_pg = tuf[0]
                name_mdb = [
                    attr for attr in attrs_details if attrs_details[attr]["name_conf"] == name_pg][0]
                type_orig = tuf[1].lower()
                type_new = attrs_details[name_mdb]["type_conf"].lower()
                attrs_details[name_mdb]["type_conf"] = type_orig
                logger.warning(
                    "[EXTRACTOR] Type conversion failed for column '%s'. Skipping conversion %s -> %s." % (name_pg, type_orig, type_new))
